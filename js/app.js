(() => {
    ...
    function initSliders() {
        ...
        if (document.querySelector(".card-catalog__slider")) new core(".card-catalog__slider", {
            slidesPerView: 1,
            modules: [ Navigation, Pagination ],
            observer: true,
            observeParents: true,
        ...
        if (document.querySelector(".bouquets__slider")) new core(".bouquets__slider", {
            slidesPerView: 1.1,
            spaceBetween: 20,
            modules: [ Navigation, Pagination ],
            observer: true,
    ...
    likeBtn.forEach((fovoriteBtn => {
        fovoriteBtn.addEventListener("click", (() => {
            if (!fovoriteBtn.classList.contains("red")) {
                ...
                counter--;
                headerLabel.textContent = counter;
                if (0 === counter) headerLabel.style.display = "none";
            }
            console.log(counter);
        }));
    }));
    ...
    try {
        likeBtn.forEach((btn => {
            btn.addEventListener("click", (({target}) => {
                if (target.classList.contains("card-catalog__favorite-button")) btn.classList.toggle("green"); else btn.classList.toggle("red");
            }));
        }));
        viewBtn.forEach((btn => {
            btn.addEventListener("click", (({target}) => {
                if (target.classList.contains("card-catalog__view-button")) btn.classList.toggle("active");
            }));
        }));
        cardList.addEventListener("click", (({target}) => {
            if (target.closest(".card-catalog__view-button")) cardDesc.forEach(((item, i) => {
                item.classList.toggle("active");
                if (cardCatalogLabels[i]) cardCatalogLabels[i].classList.toggle("active");
            }));
        }));
    } catch (error) {
        console.log(error);
